...
const COLLEGE_PROGRAMS: Record<string, string[]> = {
  ...
  "ABM": ["BS Accountancy"],
  "CAS": ["AB Communication", "AB Political Science", "BS Psychology", "BS Biology"],
  "CBA": ["BS Business Administration", "BS Entrepreneurship", "BS Tourism Management"],
  "CEA": ["BS Architecture", "BS Civil Engineering", "BS Electrical Engineering", "BS Mechanical Engineering", "BS Electronics Engineering", "BS Industrial Engineering"],
...
export default function CheckInPage({ params, searchParams }: PageProps) {
  ...
  useEffect(() => {
    if (!loading) {
      ...
      const isAllowedAdmin = ['francesaly11@gmail.com', 'jcesperanza@neu.edu.ph', 'admin@example.com'].some(admin => admin.toLowerCase() === email.toLowerCase());
      if (!isInstitutional && !isAllowedAdmin) router.push('/');
    }
  }, [loading, user, router]);

  useEffect(() => {
    const list = Object.entries(DEPARTMENTS).map(([id, name]) => ({ id, name }));
    setColleges(list);
  }, []);

  useEffect(() => {
    if (profile?.visitorType && profile.visitorType !== 'Unassigned') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile && profile.visitorType !== visitorType) {
        await updateDoc(doc(firestore, "users", user.uid), { visitorType: visitorType });
      }
  ...
  const getGreetingName = () => {
    ...
    return parts.length >= 2 ? `${parts[0]} ${parts[1]}` : (parts[0] || "Student");
  };

  const isEmployee = visitorType === "Employee (Faculty/Staff)";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-background relative overflow-hidden transition-colors duration-300">
        <div className="fixed inset-0 z-0">
          {bgImage && <Image src={bgImage} alt="Library" fill className="object-cover opacity-60 dark:opacity-40" priority data-ai-hint="university building" />}
        ...
        <Card className="w-full max-w-lg overflow-hidden border-none shadow-2xl animate-in zoom-in-95 duration-500 relative z-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl">
          ...
          <CardContent className="pt-12 pb-10 px-8 text-center space-y-6">
            ...
            <div className="space-y-4 pt-6">
              ...
              <Button variant="default" size="lg" className="w-full rounded-xl shadow-lg font-bold text-lg" onClick={() => router.push('/')}>Done</Button>
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background relative overflow-hidden transition-colors duration-300">
      <div className="fixed inset-0 z-0 pointer-events-none">
        {bgImage && <Image src={bgImage} alt="Library" fill className="object-cover opacity-60 dark:opacity-45" priority data-ai-hint="university building" />}
      ...
      <header className="p-4 md:p-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b dark:border-slate-800 sticky top-0 z-40 transition-colors">
        <div className="max-w-7xl mx-auto flex items-center justify-between w-full">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 relative"><Image src={logoImage} alt="Logo" fill className="object-contain" data-ai-hint="university logo" /></div>
          ...
          <div className="flex items-center gap-4">
            {profile?.role === 'admin' && (
              <Button variant="outline" size="sm" asChild className="hidden md:flex gap-2 border-primary/20 text-primary hover:bg-primary/5 rounded-xl dark:border-slate-800">
                <Link href="/admin/dashboard"><ShieldCheck className="h-4 w-4" />Admin Portal</Link>
              </Button>
            )}
            <ThemeToggle />
            <Button variant="ghost" size="icon" onClick={logOut} className="text-slate-400 hover:text-primary transition-all rounded-xl"><LogOut className="h-5 w-5" /></Button>
          </div>
        </div>
      </header>
            ...
            <Card className="border-none shadow-2xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl overflow-hidden">
              <CardHeader className="space-y-1 pb-8">
                ...
                <CardTitle className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">
                  {isEmployee ? "Employee Check-in" : "Student Check-in"}
                </CardTitle>
                <CardDescription className="text-slate-600 dark:text-slate-400">Please provide your visit details below to enter the library.</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-2">
                    <Label className="text-xs font-bold uppercase tracking-wider text-slate-400">Visitor Classification</Label>
                    <div className="relative">
                      <Select value={visitorType} onValueChange={(v) => setVisitorType(v as VisitorType)} required>
                        <SelectTrigger className="h-12 border-slate-200 dark:border-slate-800 bg-white/50 dark:bg-slate-800/50 focus:ring-primary pl-10 rounded-xl">
                  ...
                  <Button type="submit" className="w-full h-12 text-lg font-bold shadow-lg rounded-xl transition-all" disabled={actionLoading || !purpose || !collegeId || !program || !visitorType}>
          ...
          <div className="space-y-6">
            <Card className="border-none shadow-xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-2xl overflow-hidden">
              <CardHeader className="pb-2 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                <CardTitle className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-2"><Clock className="h-4 w-4 text-primary/60" />Library Information</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6 pt-6 text-sm">
                <div className="space-y-3">
                  <p className="text-[10px] font-bold text-primary uppercase tracking-widest">Operation Hours</p>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800"><span className="text-slate-500 font-medium text-xs">Mon - Fri</span><span className="font-bold text-slate-900 dark:text-white text-xs">7:00 AM - 7:00 PM</span></div>
                    <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800"><span className="text-slate-500 font-medium text-xs">Sat</span><span className="font-bold text-slate-900 dark:text-white text-xs">7:00 AM - 6:00 PM</span></div>
                  </div>
                </div>
              </CardContent>
            </Card>
